<template>
    <div class="flex h-screen">
        <aside 
            ...
            style="background: linear-gradient(180deg, #0d0d3b 0%, #141452 50%, #1a1a6e 100%); color: #ffffff;"
            @mouseenter="handleSidebarHover(true)"
            @mouseleave="handleSidebarHover(false)"
        >
            <!-- Logo Section -->
            <div class="flex items-center justify-center py-2 px-3 border-b border-[#d4a843]/20">
                <div v-if="(sidebarExpanded || (isHovered && !isPinned))">
                    <img src="/Dethro.jpg" alt="Dethro Technology" class="w-24 rounded-lg" />
                </div>
                <div v-else class="flex flex-col items-center mx-auto">
                    <img src="/Dethro.jpg" alt="Dethro Technology" class="w-10 rounded-md" />
                </div>
            </div>
            ...
            <nav class="flex-1 p-4 space-y-2">
                <!-- Dashboard -->
                <NuxtLink 
                    class="menu-item group flex items-center py-3 rounded-xl text-white hover:bg-[#d4a843]/15 hover:shadow-lg font-medium transition-all duration-200" 
                ...
                >
                    <div class="flex items-center justify-center w-8 h-8 bg-[#d4a843]/15 rounded-lg group-hover:bg-[#d4a843]/25 transition-colors">
                        <Icon name="lucide:layout-dashboard" class="h-5 w-5 text-[#d4a843]"/>
                    </div>
                    <div v-if="(sidebarExpanded || (isHovered && !isPinned))" class="flex flex-col ml-3">
                        <span class="text-sm font-medium">Dashboard</span>
                        <span class="text-xs text-white/50">Overview</span>
                    </div>
                </NuxtLink>
                ...
                >
                    <div class="flex items-center justify-center w-8 h-8 bg-[#d4a843]/15 rounded-lg group-hover:bg-[#d4a843]/25 transition-colors">
                        <Icon name="lucide:receipt" class="h-5 w-5 text-[#d4a843]"/>
                    </div>
                    <div v-if="(sidebarExpanded || (isHovered && !isPinned))" class="flex flex-col ml-3">
                        <span class="text-sm font-medium">Transactions</span>
                        <span class="text-xs text-white/50">Payment History</span>
                    </div>
                </NuxtLink>
                
                <!-- Meters -->
                <NuxtLink 
                    class="menu-item group flex items-center px-4 py-3 rounded-xl text-white hover:bg-[#d4a843]/15 hover:shadow-lg font-medium transition-all duration-200" 
                    :class="(sidebarExpanded || (isHovered && !isPinned)) ? 'justify-start' : 'justify-center'"
                    @click="navigateTo(`/my/${$route.params.customer_id}/meters`)"
                >
                    <div class="flex items-center justify-center w-8 h-8 bg-[#d4a843]/15 rounded-lg group-hover:bg-[#d4a843]/25 transition-colors">
                        <Icon name="lucide:gauge" class="h-5 w-5 text-[#d4a843]"/>
                    </div>
                    <div v-if="(sidebarExpanded || (isHovered && !isPinned))" class="flex flex-col ml-3">
                        <span class="text-sm font-medium">Meters</span>
                        <span class="text-xs text-white/50">Manage Devices</span>
                    </div>
                </NuxtLink>
                ...
                >
                    <div class="flex items-center justify-center w-8 h-8 bg-[#d4a843]/15 rounded-lg group-hover:bg-[#d4a843]/25 transition-colors">
                        <Icon name="lucide:bar-chart-3" class="h-5 w-5 text-[#d4a843]"/>
                    </div>
                    <div v-if="(sidebarExpanded || (isHovered && !isPinned))" class="flex flex-col ml-3">
                        <span class="text-sm font-medium">Statement</span>
                        <span class="text-xs text-white/50">Reports & Analytics</span>
                    </div>
                </NuxtLink>
                ...
                <div 
                    v-if="isAdmin"
                    class="menu-item group flex items-center py-3 rounded-xl text-white hover:bg-[#d4a843]/15 hover:shadow-lg font-medium transition-all duration-200 cursor-pointer" 
                    :class="(sidebarExpanded || (isHovered && !isPinned)) ? 'justify-start px-4' : 'justify-center px-2'"
                    @click="goToAdmin"
                >
                    <div class="flex items-center justify-center w-8 h-8 bg-[#d4a843]/15 rounded-lg group-hover:bg-[#d4a843]/25 transition-colors">
                        <Icon name="lucide:shield-check" class="h-5 w-5 text-[#d4a843]"/>
                    </div>
                    <div v-if="(sidebarExpanded || (isHovered && !isPinned))" class="flex flex-col ml-3">
                        <span class="text-sm font-medium">Admin</span>
                        <span class="text-xs text-white/50">System Management</span>
                    </div>
                </div>
            </nav>
            
            <!-- Customer Support Section -->
             <div class="p-4 border-t border-[#d4a843]/20">
                 <NuxtLink 
            ...
            <div v-if="isAdmin" class="p-4 border-t border-[#d4a843]/20">
                ...
                >
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 bg-[#d4a843]/20 rounded-full flex justify-center items-center">
                            <Icon name="lucide:user" class="h-4 w-4 text-[#d4a843]" />
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-white truncate">Customer Portal</p>
                            <p class="text-xs text-white/50 truncate">Utility Management</p>
                        </div>
                    </div>
                </div>
